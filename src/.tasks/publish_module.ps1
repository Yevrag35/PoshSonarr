...
[string[]] $CopyToOutput = Get-Content -Path $BuildDependenciesJson -Raw | ConvertFrom-Json -Depth 10 | ForEach-Object -MemberName CopyToOutput
...
foreach ($toCopy in $CopyToOutput) {
	$dependency = $targets.psobject.Properties.Where({ $_.Name -like "$toCopy*" }) | Select-Object -First 1
...
$manifestArgs = @{
	...
	IconUri              = 'https://raw.githubusercontent.com/Yevrag35/PoshSonarr/refs/heads/master/.img/PoshSonarr.png'
